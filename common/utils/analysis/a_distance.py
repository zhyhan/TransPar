...
# def density_ratio(source_feature: torch.Tensor, target_feature: torch.Tensor,
#               device, progress=True, training_epochs=10):
#     """
#     Calculate the :math:`\mathcal{A}`-distance, which is a measure for distribution discrepancy.
...
#     feature = torch.cat([source_feature, target_feature], dim=0)
#     label = torch.cat([source_label, target_label], dim=0)
#     sample_ratio = len(target_label)/len(source_label)#TODO
#     dataset = TensorDataset(feature, label)
#     length = len(dataset)
#     train_size = int(0.8 * length)
#     val_size = length - train_size
...
#         anet.eval()
#         #collect the probability of each example belongings to the source domain. 
#         first_test = True
#         meter = AverageMeter("accuracy", ":4.2f")
#         with torch.no_grad():
#             for (x, label) in val_loader:
...
#             else:
#                 w.append(0.5)
#         ratio = torch.mean(torch.tensor(w))*sample_ratio
#         print(torch.mean(torch.tensor(w)), sample_ratio, ratio)
#         m = nn.Sigmoid()
#         ratio = m(ratio)
#         error = 1 - meter.avg / 100
#         a_distance = 2 * (1 - 2 * error)
#         if progress:
#             print("epoch {} accuracy: {} A-dist: {} Density-ratio: {}".format(epoch, meter.avg, a_distance, ratio))
