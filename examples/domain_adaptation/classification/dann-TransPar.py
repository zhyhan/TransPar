...
def main(args: argparse.Namespace):
    logger = CompleteLogger(args.log, args.keep_ratio, args.phase)
    print(args)
    ...
    # define optimizer and lr scheduler#TODO
    optimizer = SGD(classifier.get_parameters() + domain_discri.get_parameters(),
                    args.lr, momentum=args.momentum, weight_decay =args.weight_decay, nesterov=False)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
    ...
    for i in range(args.iters_per_epoch):
        ...
        # compute entropy minimization loss on target domain
        em_loss = entropy(y_t)
        transfer_loss = domain_adv(f_s, f_t)
        domain_acc = domain_adv.domain_discriminator_accuracy
        loss = cls_loss + transfer_loss * args.trade_off + 0.1*em_loss
        ...
        optimizer.zero_grad()
        loss.backward()
        #LTH(model, keep_ratio = keep_ratio, is_classifier=True)
        LTH(model.backbone, keep_ratio = keep_ratio, is_classifier=True)
        LTH(model.bottleneck, keep_ratio = keep_ratio, is_classifier=True)
        LTH(model.head, keep_ratio = keep_ratio, is_classifier=True)
        LTH(domain_adv, keep_ratio = 1-keep_ratio, is_classifier=False)
...
def LTH(model, is_classifier=True, keep_ratio = 0.5, eps = 1e-10):
    grads = dict()
