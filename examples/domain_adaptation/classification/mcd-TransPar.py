...
def main(args: argparse.Namespace):
    ...
    for epoch in range(args.epochs):
        # train for one epoch
        train(train_source_iter, train_target_iter, G, F1, F2, optimizer_g, optimizer_f, epoch, args, keep_ratio)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
          ...
          optimizer_g: SGD, optimizer_f: SGD, epoch: int, args: argparse.Namespace, keep_ratio: float):
    ...
    for i in range(args.iters_per_epoch):
        ...
        loss.backward()
        LTH(G, keep_ratio = keep_ratio)
        LTH(F1, keep_ratio = keep_ratio)
        LTH(F2, keep_ratio = keep_ratio)
