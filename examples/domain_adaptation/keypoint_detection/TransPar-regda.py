...
def train(train_source_iter, train_target_iter, model, criterion,regression_disparity,
    ...
    for i in range(args.iters_per_epoch):
        ...
        loss_s.backward()
        LTH(model.head, keep_ratio = keep_ratio)
        optimizer_f.step()
        optimizer_h.step()
        ...
        LTH(model.head_adv, keep_ratio = 1. - keep_ratio)
        optimizer_h_adv.step()
        ...
        LTH(model.backbone, keep_ratio = keep_ratio)
        #LTH(model.upsampling, keep_ratio = keep_ratio)
        optimizer_f.step()
